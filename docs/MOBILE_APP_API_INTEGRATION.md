...
- `price` is the price of one base `unit` (for example, one `PIECE`).
- To define a bale, send `package_unit: "BALE"` and `units_per_package` greater than 1 together. For example, `unit: "PIECE"` and `units_per_package: 15` means one bale contains 15 pieces.
